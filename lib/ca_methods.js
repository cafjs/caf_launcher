...
const minimalState = (self) => {
    return {
        apps: self.state.apps,
        cacheKeys: self.state.cacheKeys
    };
};

exports.methods = {
    async __ca_init__() {
        this.$.security.addRule(this.$.security.newSimpleRule('ensureUp'));
        this.$.security.addRule(
            this.$.security.newSimpleRule('__external_ca_touch__')
        );
        this.state.apps = {};
    ...
    async __ca_pulse__() {
        ...
        if (nowInSec - this.state.lastWakeupInSec >
            ...
            launcherUtil.wakeupApps(this); // do not await, best effort
            if (launcherUtil.isAdmin(this)) {
                // root-launcher CAs for all users should be up too...
                this.ensureAllUp();
            }
    ...
    async ensureUp() {
        this.$.log && this.$.log.trace(
            `ensureUp() CA: ${this.__ca_getName__()}`
        );
        /*
         * Can be called by the 'nobody' user, i.e., no authentication token,
         * and it brings back the CA from hibernation after this app restarts.
         *
         * This ensures that tokens refresh in `__ca_pulse__`, even if the user
         * does not login.
        */
        return [];
    },
    async ensureAllUp() {
        assert(launcherUtil.isAdmin(this));
        launcherUtil.ensureAllUp(this); // best effort
        ...
        try {
            ...
            if (ca[0] !== splitMe[0]) {
                ...
                if (units < 1) {
                    ...
                    notifyWebApp(this, minimalState(this));
                    return this.refreshTokens();
                }
            }
        } catch (err) {
    ...
    async shareToken(from, to) {
        try {
            await launcherUtil.shareToken(this, from, to);
            return this.getStateApps();
        } catch (err) {
            return [err];
        }
