...
const SLACK_IN_MSEC = 3600000; // 1 hour minimum duration
...
const findUsers = async (self) => {
    const adminUserApp = json_rpc.joinNameArray([
        USER_APP,
        json_rpc.joinName(ADMIN, CA_ADMIN_USER_APP)
    ], json_rpc.APP_SEPARATOR);

    const token = self.state.apps[adminUserApp];
    if (token) {
        const [err, result] = await self.$.crossapp.dirtyCall(
            adminUserApp, null, 'filterUsers', [null], token, 0 // no retry
        );
        if (err) {
            self.$.log && self.$.log.debug(
                `findUsers()  err:` + myUtils.errToPrettyStr(err)
            );
        }
        return result || [];
    } else {
        self.$.log && self.$.log.debug(
            `findUsers() no token for ${adminUserApp}`
        );
        return [];
    }
};

const ensureUp = async (self, user, desktop) => {
    try {
        const app = json_rpc.joinNameArray([
            self.__ca_getAppName__(),
            json_rpc.joinName(user, desktop)
        ], json_rpc.APP_SEPARATOR);
        self.$.log && self.$.log.trace(`util_ensureUp() user: ${user} ` +
                                       `desktop: ${desktop}`);
        return await self.$.crossapp.dirtyCall(
            app, json_rpc.DEFAULT_FROM, 'ensureUp', [], null, 0 // no retry
        );
    } catch (err) {
        // ignore, best effort
        self.$.log && self.$.log.trace(
            `util_ensureUp() user: ${user} desktop: ${desktop} err:` +
                myUtils.errToPrettyStr(err)
        );
    }
};

exports.ensureAllUp = async function(self) {
    try {
        const users = await findUsers(self);
        self.$.log && self.$.log.trace(`ensureAllUp() users: ${users}`);
        users.forEach((user) => {
            DESKTOPS.forEach((desktop) => {
                ensureUp(self, user, desktop); // best effort, no await
            });
        });
    } catch (err) {
        self.$.log && self.$.log.debug(
            'Cannot `ensureAllUp`, exception: ' + myUtils.errToPrettyStr(err)
        );
    }
};
...
exports.shareToken = function(self, from, to) {
    const fromToken = self.state.apps[from];
    assert(fromToken, `Missing 'from' token ${from}`);
    const toToken =  self.state.apps[to];
    assert(toToken, `Missing 'to' token ${to}`);
    return self.$.crossapp.dirtyCall(
        to, null, 'receiveToken', [from, fromToken], toToken, 0 // no retry
    );
};
